...
# Remove .d.ts generated by prisma/client to avoid bun parsing them as code
RUN find /app/node_modules -name '*.d.ts' -path '*/@prisma/client/*' -exec rm -f {} +

# pm2 ecosystem config (embedded to avoid Coolify context issues)
# Use tsx to run TypeScript with path aliases
RUN echo '{"apps":[{"name":"bizidashboard-jobs","script":"tsx","args":"src/jobs/standalone.ts","cwd":"/app","instances":1,"autorestart":true,"watch":false,"max_memory_restart":"1G","env":{"NODE_ENV":"production"}}]}' > /app/ecosystem.config.js
