...
import { a as getStationPatternsBulk, l as getLocalBucket, s as getStationsWithLatestStatus } from "./read-BgY-Mwu8.js";
import { d as prisma, t as withCache } from "./cache-DMRFuswD.js";
import { n as buildStationDistrictMap, r as fetchDistrictCollection } from "./districts-e09LFoic.js";
import { n as haversineDistanceMeters } from "./geo-C-dxtd4-.js";
import "server-only";
...
function computeConfidence(currentPattern, futurePattern) {
	...
	return Number(clamp$1(confidence, .42, .92).toFixed(2));
}
function predictPoint(context, horizonMinutes, now) {
	...
	const currentRatio = clamp$1(context.bikesAvailable, 0, capacity) / capacity;
	const currentPatternRatio = clamp$1(currentPattern?.occupancyAvg ?? currentRatio, 0, 1);
	const expectedDelta = clamp$1(futurePattern.occupancyAvg, 0, 1) - currentPatternRatio;
	const recencyWeight = horizonMinutes === 30 ? .7 : .48;
	const predictedBikes = roundPrediction(clamp$1(clamp$1(currentRatio * recencyWeight + (currentRatio + expectedDelta) * (1 - recencyWeight), 0, 1) * capacity, 0, capacity));
	return {
		horizonMinutes,
		predictedBikesAvailable: predictedBikes,
		predictedAnchorsFree: roundPrediction(clamp$1(capacity - predictedBikes, 0, capacity)),
		confidence: computeConfidence(currentPattern, futurePattern)
	};
}
