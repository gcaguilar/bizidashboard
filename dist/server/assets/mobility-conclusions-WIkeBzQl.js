import { i as isDistrictCollection, n as buildStationDistrictMap, t as DISTRICTS_GEOJSON_URL } from "./districts-DMcc_jOx.js";
import { n as captureWarningWithContext } from "./sentry-reporting-6fzVQr1k.js";
import { t as TIMEZONE } from "./timezone-DIvdn6H4.js";
import { n as getMonthBounds, r as isValidMonthKey, t as formatMonthLabel } from "./months-CotCm8RF.js";
import { f as sql, r as withCache, s as prisma } from "./cache-CQ9JHJ0b.js";
import { _ as getCoverageSummary } from "./shared-data-DRRoN2Of.js";
...
function buildConclusionsRange(monthKey) {
	if (monthKey && isValidMonthKey(monthKey)) {
		...
		return {
			currentDaily: sql`"bucketDate" >= ${start}::timestamp AND "bucketDate" < ${endExclusive}::timestamp`,
			previousDaily: sql`"bucketDate" >= ${previousStart}::timestamp AND "bucketDate" < ${start}::timestamp`,
			currentHourly: sql`"bucketStart" >= ${start}::timestamp AND "bucketStart" < ${endExclusive}::timestamp`,
			topStationsDaily: sql`"DailyStationStat"."bucketDate" >= ${start}::timestamp AND "DailyStationStat"."bucketDate" < ${endExclusive}::timestamp`,
			summaryScope: `en ${formatMonthLabel(monthKey)}`,
			comparisonScope: "vs mes previo"
		};
	}
	return {
		currentDaily: sql`"bucketDate" >= CURRENT_DATE - INTERVAL '6 days'`,
		previousDaily: sql`"bucketDate" >= CURRENT_DATE - INTERVAL '13 days' AND "bucketDate" < CURRENT_DATE - INTERVAL '6 days'`,
		currentHourly: sql`"bucketStart" >= CURRENT_DATE - INTERVAL '6 days'`,
		topStationsDaily: sql`"DailyStationStat"."bucketDate" >= CURRENT_DATE - INTERVAL '29 days'`,
...
async function buildMobilityConclusionsPayload(dateKey, monthKey) {
	...
	const summary = demandDeltaRatio === null ? `La ciudad acumula ${Math.round(demandLast7Days)} puntos de demanda agregada ${range.summaryScope}, con ocupacion media del ${Math.round(occupancyLast7Days * 100)}% sobre ${sharedCoverage.totalDays} dias historicos disponibles.` : `La demanda ${selectedMonth ? "mensual" : "semanal"} se mueve ${formatDelta(demandDeltaRatio)} ${range.comparisonScope} y la ocupacion media se situa en ${Math.round(occupancyLast7Days * 100)}% (${formatDelta(occupancyDeltaRatio)}).`;
	const highlights = [
		{
			title: selectedMonth ? "Demanda del mes" : "Demanda semanal",
			detail: `${Math.round(demandLast7Days)} puntos ${selectedMonth ? `en ${formatMonthLabel(selectedMonth)}` : "en 7 dias"} (${formatDelta(demandDeltaRatio)} ${range.comparisonScope}).`
		},
		{
...
//#endregion
export { getDailyMobilityConclusions as t };

//# sourceMappingURL=mobility-conclusions-WIkeBzQl.js.map