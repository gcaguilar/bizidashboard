...
import { fetchTransitHeatmap } from '@/lib/transit-api';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const payload = await fetchTransitHeatmap(mode, transitStopId);
    ...
    console.error('[API Transit Heatmap] Error fetching heatmap:', error);
    return NextResponse.json({ error: 'Failed to fetch transit heatmap' }, { status: 500 });
  }
}
