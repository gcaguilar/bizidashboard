...
export const Route = createFileRoute('/api/ops/sentry-test/')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          return await handleOperationalRoute(
            request,
            {
            ...
            async ({ headers }) => {
              ...
              if (!dsnSource) {
                return new Response(JSON.stringify({ success: false, error: 'Sentry DSN is not configured on the server runtime.', timestamp: new Date().toISOString() }), { status: 503, headers: { 'Content-Type': 'application/json', ...headers, 'Cache-Control': 'no-store' } })
              }
              ...
              return new Response(JSON.stringify({ success: true, eventId, flushed, marker, dsnSource, timestamp: new Date().toISOString() }), { status: 202, headers: { 'Content-Type': 'application/json', ...headers, 'Cache-Control': 'no-store' } })
            }
          )
        } catch (error) {
