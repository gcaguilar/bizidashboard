...
const HEATMAP_WATERMARK = 'transit-heatmap-rollup';
...
export async function runTransitHeatmapRollup(cutoff: Date): Promise<RollupResult> {
  ...
  for (const stat of hourlyStats) {
    ...
    const key = `${stat.transitStopId}-${dayOfWeek}-${hour}`;
  ...
  if (rows.length > 0) {
    await forEachSqliteBatch(rows, 7, async (rowChunk) => {
      const values = rowChunk.map((row) =>
        Prisma.sql`(${row.transitStopId}, ${row.provider}, ${row.dayOfWeek}, ${row.hour}, ${row.etaAvg}, ${row.arrivalPressureAvg}, ${row.arrivalEventsAvg}, ${row.staleRate}, ${row.sampleCount})`
      );

      await prisma.$executeRaw`
        INSERT INTO TransitStopHeatmapCell (
          transitStopId,
          provider,
          dayOfWeek,
          hour,
        ...
        ON CONFLICT(transitStopId, dayOfWeek, hour) DO UPDATE SET
    ...
    await setWatermark(HEATMAP_WATERMARK, cutoff);
  }
