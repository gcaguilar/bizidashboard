...
- OAuth bearer tokens are verified against Auth0's JWKS (`AUTH0_DOMAIN`, `AUTH0_AUDIENCE`) — see `src/lib/oauth.ts`. Rate limiting is applied per Auth0 `client_id`, using a per-client override (`ApiClient.customRateLimit`/`customRateWindow`) when configured, otherwise the default in `src/lib/security/api-clients.ts`. Provisioning uses a separate Auth0 M2M application scoped to the Management API (`AUTH0_MGMT_CLIENT_ID`/`AUTH0_MGMT_CLIENT_SECRET`).
