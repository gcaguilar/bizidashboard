...
- The developer login itself uses a second, distinct Auth0 application (`AUTH0_LOGIN_CLIENT_ID`/`AUTH0_LOGIN_CLIENT_SECRET`, type "Regular Web Application" — confidential client, code exchange happens server-side in `/api/auth/callback`). The logged-in session is a TanStack Start sealed cookie (`SESSION_SECRET`, min 32 chars) — see `src/lib/auth/developer-session.ts` and `src/lib/auth/auth0-web.ts`. This is unrelated to the M2M application used for provisioning.
