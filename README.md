...
- Third-party API consumers get their key from the `/developers` page: they log in via Auth0 Universal Login (Authorization Code flow, `/api/auth/login` → `/api/auth/callback`), then `POST /api/developer/register` (session-gated, rate-limited per IP) mints an API key bound to their verified email. The response includes the full key (`biz_live_…`, shown once) and its rate limit; it is sent as `x-api-key: <key>` on elevated public routes. There is no token exchange step and no OAuth client to configure — one header is the whole contract.
...
- Developers list their own live keys at `GET /api/developer/keys` (prefix and usage metadata only, never the secret) and revoke one at `POST /api/developer/revoke` (`{ keyId }`, session-gated, ownership-checked against the session email) — both drive the `/developers` page UI. Revocation takes effect on the next request, since every call re-validates the key against the database.
- The developer login uses an Auth0 application of type "Regular Web Application" (`AUTH0_LOGIN_CLIENT_ID`/`AUTH0_LOGIN_CLIENT_SECRET` — confidential client, code exchange happens server-side in `/api/auth/callback`). The logged-in session is a TanStack Start sealed cookie (`SESSION_SECRET`, min 32 chars, `SameSite=Lax`/`Secure`) — see `src/lib/auth/developer-session.ts` and `src/lib/auth/auth0-web.ts`. It only gates key management; it is never accepted as API credentials.
