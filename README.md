...
- Third-party API consumers self-register from the `/developers` page: they log in via Auth0 Universal Login (Authorization Code flow, `/api/auth/login` → `/api/auth/callback`), then `POST /api/developer/register` (session-gated, rate-limited per IP) provisions an Auth0 Machine-to-Machine application under their verified email. The response includes `clientId`/`clientSecret` (shown once) and the Auth0 token URL. Clients exchange those credentials for a bearer token directly with Auth0 (`client_credentials` grant) and send it as `Authorization: Bearer <token>` on elevated public routes instead of `X-Public-Api-Key`.
- OAuth bearer tokens are verified against Auth0's JWKS (`AUTH0_DOMAIN`, `AUTH0_AUDIENCE`) and must carry the `public_api.read` scope — see `src/lib/oauth.ts`. The `AUTH0_AUDIENCE` API must have `public_api.read` defined as a scope in Auth0 (Applications → APIs → your API → Scopes) before provisioning will succeed. `ApiClient` acts as an allow-list, not just a rate-limit cache: a valid Auth0 token whose `client_id` has no matching (non-revoked) `ApiClient` row is rejected with 401, even if Auth0 itself considers the token valid. Rate limiting is applied per Auth0 `client_id`, using a per-client override (`ApiClient.customRateLimit`/`customRateWindow`) when configured, otherwise the default in `src/lib/security/api-clients.ts`. Provisioning uses a separate Auth0 M2M application scoped to the Management API (`AUTH0_MGMT_CLIENT_ID`/`AUTH0_MGMT_CLIENT_SECRET`).
- Developers can revoke their own API client at `POST /api/developer/revoke` (`{ auth0ClientId }`, session-gated, ownership-checked against the session email) — also available from the `/developers` page UI. Revoking deletes the Auth0 application and flips `ApiClient.isActive` off; any already-issued access token for that client keeps working until it naturally expires (Auth0's default M2M token TTL), since verification is JWT-based rather than a live Auth0 lookup — the `ApiClient` check on every request is what actually blocks further use.
- The developer login itself uses a second, distinct Auth0 application (`AUTH0_LOGIN_CLIENT_ID`/`AUTH0_LOGIN_CLIENT_SECRET`, type "Regular Web Application" — confidential client, code exchange happens server-side in `/api/auth/callback`). The logged-in session is a TanStack Start sealed cookie (`SESSION_SECRET`, min 32 chars, `SameSite=Lax`/`Secure`) — see `src/lib/auth/developer-session.ts` and `src/lib/auth/auth0-web.ts`. This is unrelated to the M2M application used for provisioning.
- `POST /api/developer/register` and `POST /api/developer/revoke` are session-gated through the shared `requireDeveloperSession()` guard and reject cross-origin requests as CSRF protection: if the request carries an `Origin` header, it must match the request's own `Host` (`rejectCrossOriginRequest()` in `src/lib/security/http.ts`) or the request is rejected with 403.
- `GET /.well-known/oauth-protected-resource` serves OAuth Protected Resource Metadata (RFC 9728) — the endpoint advertised in the `WWW-Authenticate` header on 401s from OAuth-protected routes.
...
- Production startup validates critical runtime configuration, including `JWT_SECRET`, `SIGNATURE_SECRET`, `OPS_API_KEY`/`COLLECT_API_KEY`, `PUBLIC_API_KEY` (if set), `REDIS_URL`, and a valid `APP_URL`; when Auth0 is configured it also requires the matching `AUTH0_AUDIENCE`/`AUTH0_MGMT_CLIENT_ID`/`AUTH0_MGMT_CLIENT_SECRET` (public API) and `AUTH0_LOGIN_CLIENT_SECRET`/`SESSION_SECRET` (developer login) — see `src/lib/security/config.ts`. Outside production, the same secrets are checked against known dev-default/placeholder values and logged as a warning rather than failing startup.
